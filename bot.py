...
from config import (
    ...
    XUI_INBOUND_ID, XUI_PUBLIC_KEY, XUI_SERVER_PORT, XUI_SHORT_ID, XUI_SNI,
)
...
async def give(message: types.Message):
    ...
    async with async_session() as session:
        ...
        user.balance += amount
        await session.commit()
        await message.answer(f"Баланс {uid}: {user.balance:.2f} ₽")
...
async def create_vless(message: types.Message):
    ...
    try:
        ...
        if client_uuid:
            try: await delete_vless_client(client_uuid)
...
async def subscriptions(message: types.Message):
    ...
    for t in tunnels:
        ...
        if t.sub_type == "vless":
            title, key = "🌐 VLESS", vless_uri(t.backend_id, t.meta_info)
        else:
            title, key = "📞 olcRTC", f"olcrtc://{SERVER_IP}:{t.port}?room={quote(t.meta_info, safe='')}"
...
async def renew(callback: types.CallbackQuery):
    ...
    async with async_session() as session:
        ...
        try:
            ...
            user.balance -= TUNNEL_COST
            t.expires_at = new_expiry; t.traffic_used_bytes = 0
            await session.commit()
        except Exception as exc:
            await session.rollback()
